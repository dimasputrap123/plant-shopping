...
const productSlice = createSlice({
  ...
  reducers: {
    decreaseProductStock: (
      state,
      action: PayloadAction<{ name: string; stock: number }>
    ) => {
      state.listProduct = state.listProduct.map((e) => {
        if (e.name === action.payload.name) {
          e.stocks -= action.payload.stock;
        }
        return e;
      });
    },
...
export const { decreaseProductStock } = productSlice.actions;
